...
static esp_err_t gc0308_write_mirror_reg(esp_cam_sensor_device_t *dev, int enable)
{
    esp_err_t ret = ESP_FAIL;
    ret = gc0308_select_page(dev, 0x00);
...
static esp_err_t gc0308_write_vflip_reg(esp_cam_sensor_device_t *dev, int enable)
{
    esp_err_t ret = ESP_FAIL;
    ret = gc0308_select_page(dev, 0x00);
...
static esp_err_t gc0308_write_orientation_reg(esp_cam_sensor_device_t *dev, int hmirror, int vflip)
{
    const uint8_t mode1_bits = (uint8_t)((hmirror ? 0x01 : 0x00) | (vflip ? 0x02 : 0x00));
    esp_err_t ret = gc0308_select_page(dev, 0x00);
    ret |= gc0308_set_reg_bits(dev->sccb_handle, GC0308_REG_CISCTL_MODE1, 0, 0x02, mode1_bits);
    return ret;
}

static esp_err_t gc0308_set_mirror(esp_cam_sensor_device_t *dev, int enable)
{
    struct gc0308_cam *cam_gc0308 = (struct gc0308_cam *)dev->priv;
    esp_err_t ret = gc0308_write_mirror_reg(dev, enable);
    if (ret == ESP_OK && cam_gc0308 != NULL) {
        cam_gc0308->gc0308_para.hmirror_en = enable ? 1 : 0;
...
{
    struct gc0308_cam *cam_gc0308 = (struct gc0308_cam *)dev->priv;
    esp_err_t ret = gc0308_write_vflip_reg(dev, enable);
    if (ret == ESP_OK && cam_gc0308 != NULL) {
        cam_gc0308->gc0308_para.vflip_en = enable ? 1 : 0;
...
static esp_err_t gc0308_get_para_value(esp_cam_sensor_device_t *dev, uint32_t id, void *arg, size_t size)
{
    ...
    if (arg == NULL || size != 4 || cam_gc0308 == NULL) {
        return ESP_ERR_INVALID_ARG;
    }

    switch (id) {
    case ESP_CAM_SENSOR_VFLIP: {
        *(uint32_t *)arg = cam_gc0308->gc0308_para.vflip_en;
        break;
    }
    case ESP_CAM_SENSOR_HMIRROR: {
        *(uint32_t *)arg = cam_gc0308->gc0308_para.hmirror_en;
        break;
    }
    default:
...
static esp_err_t gc0308_set_format(esp_cam_sensor_device_t *dev, const esp_cam_sensor_format_t *format)
{
    ...
    if (cam_gc0308 != NULL) {
        ret = gc0308_write_orientation_reg(dev,
                                           cam_gc0308->gc0308_para.hmirror_en,
                                           cam_gc0308->gc0308_para.vflip_en);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Reapply orientation failed");
            return ESP_CAM_SENSOR_ERR_FAILED_SET_FORMAT;
        }
        ESP_LOGI(TAG, "Reapply orientation after format: hmirror=%u vflip=%u",
                 cam_gc0308->gc0308_para.hmirror_en,
                 cam_gc0308->gc0308_para.vflip_en);
    }
...
esp_cam_sensor_device_t *gc0308_detect(esp_cam_sensor_config_t *config)
{
    ...
    cam_gc0308 = calloc(1, sizeof(struct gc0308_cam));
    if (!cam_gc0308) {

...
#define OV3660_HMIRROR_DEFAULT  (1)
#define OV3660_VFLIP_DEFAULT    (0)
#define OV3660_ORIENTATION_CTRL 0x4514
...
static bool ov3660_format_uses_binning(const esp_cam_sensor_format_t *format)
{
    if (format == NULL || format->regs == NULL) {
        return false;
    }

    const ov3660_reginfo_t *regs = (const ov3660_reginfo_t *)format->regs;
    for (int i = 0; regs[i].reg != OV3660_REGLIST_TAIL; i++) {
        if (regs[i].reg == TIMING_TC_REG20) {
            return (regs[i].val & 0x01) != 0;
        }
    }
...
static esp_err_t ov3660_write_mirror_reg(esp_cam_sensor_device_t *dev, int enable)
{
    return ov3660_set_reg_bits(dev->sccb_handle, TIMING_TC_REG21, 1, 2, enable ? 0x03 : 0x00);
}

static esp_err_t ov3660_write_vflip_reg(esp_cam_sensor_device_t *dev, int enable)
{
    return ov3660_set_reg_bits(dev->sccb_handle, TIMING_TC_REG20, 1, 2, enable ? 0x03 : 0x00);
}

static uint8_t ov3660_orientation_ctrl_value(int hmirror, int vflip, int binning)
{
    const uint8_t mode = (uint8_t)((binning ? 0x04 : 0x00)
                         | (hmirror ? 0x02 : 0x00)
                         | (vflip ? 0x01 : 0x00));
    ...
    case 0x01: /* vflip */
        return 0x88;
    case 0x02: /* hmirror */
    case 0x03: /* hmirror + vflip */
        return 0xbb;
    case 0x04: /* binning */
    case 0x07: /* binning + hmirror + vflip */
        return 0xaa;
    case 0x05: /* binning + vflip */
    case 0x06: /* binning + hmirror */
    default:
        return 0xbb;
    }
}

static esp_err_t ov3660_write_orientation_reg(esp_cam_sensor_device_t *dev, int hmirror, int vflip, int binning)
{
    esp_err_t ret = ov3660_write_mirror_reg(dev, hmirror);
    ret |= ov3660_write_vflip_reg(dev, vflip);
    ret |= ov3660_write(dev->sccb_handle, OV3660_ORIENTATION_CTRL,
                        ov3660_orientation_ctrl_value(hmirror, vflip, binning));
    return ret;
}
...
{
    struct ov3660_cam *cam_ov3660 = (struct ov3660_cam *)dev->priv;
    esp_err_t ret = ov3660_write_orientation_reg(dev,
                                                 enable,
                                                 cam_ov3660 ? cam_ov3660->ov3660_para.vflip_en : OV3660_VFLIP_DEFAULT,
                                                 cam_ov3660 ? cam_ov3660->ov3660_para.binning_en : 0);
    if (ret == ESP_OK && cam_ov3660 != NULL) {
        cam_ov3660->ov3660_para.hmirror_en = enable ? 1 : 0;
        ESP_LOGD(TAG, "Set h-mirror to: %d", enable ? 1 : 0);
    }
    return ret;
}
...
static esp_err_t ov3660_set_vflip(esp_cam_sensor_device_t *dev, int enable)
{
    struct ov3660_cam *cam_ov3660 = (struct ov3660_cam *)dev->priv;
    esp_err_t ret = ov3660_write_orientation_reg(dev,
                                                 cam_ov3660 ? cam_ov3660->ov3660_para.hmirror_en : OV3660_HMIRROR_DEFAULT,
                                                 enable,
                                                 cam_ov3660 ? cam_ov3660->ov3660_para.binning_en : 0);
    if (ret == ESP_OK && cam_ov3660 != NULL) {
        cam_ov3660->ov3660_para.vflip_en = enable ? 1 : 0;
        ESP_LOGD(TAG, "Set vflip to: %d", enable ? 1 : 0);
    }
    return ret;
}
...
static esp_err_t ov3660_query_para_desc(esp_cam_sensor_device_t *dev, esp_cam_sensor_param_desc_t *qdesc)
{
    ...
    case ESP_CAM_SENSOR_VFLIP:
        ...
        qdesc->number.step = 1;
        qdesc->default_value = OV3660_VFLIP_DEFAULT;
        break;
    case ESP_CAM_SENSOR_HMIRROR:
        ...
        qdesc->number.maximum = 1;
        qdesc->number.step = 1;
        qdesc->default_value = OV3660_HMIRROR_DEFAULT;
        break;
...
static esp_err_t ov3660_get_para_value(esp_cam_sensor_device_t *dev, uint32_t id, void *arg, size_t size)
{
    ...
    if (arg == NULL || size != 4 || cam_ov3660 == NULL) {
        return ESP_ERR_INVALID_ARG;
    }

    switch (id) {
    case ESP_CAM_SENSOR_VFLIP: {
        *(uint32_t *)arg = cam_ov3660->ov3660_para.vflip_en;
    }
    break;
    case ESP_CAM_SENSOR_HMIRROR: {
        *(uint32_t *)arg = cam_ov3660->ov3660_para.hmirror_en;
    }
    break;
    case ESP_CAM_SENSOR_AE_LEVEL: {
...
static esp_err_t ov3660_set_para_value(esp_cam_sensor_device_t *dev, uint32_t id, const void *arg, size_t size)
{
    ...
    ESP_RETURN_ON_FALSE(arg != NULL && size == 4, ESP_ERR_INVALID_ARG, TAG, "Para size err");
    int value = *(int *)arg;
...
static esp_err_t ov3660_set_format(esp_cam_sensor_device_t *dev, const esp_cam_sensor_format_t *format)
{
    ...
    if (cam_ov3660 != NULL) {
        cam_ov3660->ov3660_para.binning_en = ov3660_format_uses_binning(format) ? 1 : 0;
        ret = ov3660_write_orientation_reg(dev,
                                           cam_ov3660->ov3660_para.hmirror_en,
                                           cam_ov3660->ov3660_para.vflip_en,
                                           cam_ov3660->ov3660_para.binning_en);
        ESP_RETURN_ON_FALSE(ret == ESP_OK, ESP_CAM_SENSOR_ERR_FAILED_SET_FORMAT, TAG, "Reapply orientation failed");
        ESP_LOGI(TAG, "Reapply orientation after format: hmirror=%u vflip=%u binning=%u",
                 (unsigned)cam_ov3660->ov3660_para.hmirror_en,
                 (unsigned)cam_ov3660->ov3660_para.vflip_en,
                 (unsigned)cam_ov3660->ov3660_para.binning_en);
    }
...
esp_cam_sensor_device_t *ov3660_detect(esp_cam_sensor_config_t *config)
{
    ...
    cam_ov3660->ov3660_para.ae_target_level = OV3660_AE_LEVEL_DEFAULT;
    cam_ov3660->ov3660_para.hmirror_en = OV3660_HMIRROR_DEFAULT;
    cam_ov3660->ov3660_para.vflip_en = OV3660_VFLIP_DEFAULT;
    ...
    cam_ov3660->ov3660_para.binning_en = ov3660_format_uses_binning(dev->cur_format) ? 1 : 0;

...
// Modified for M5Stamp-S3Bat by GOB
// Original:
...
#include "index_gc0308_face_html.hpp"
...
static esp_err_t capture_handler(httpd_req_t *req)
{
    ...
    if (!detection_enabled || fb->width > 400) {
        ...
        if (fb->format == PIXFORMAT_JPEG) {
            ...
            res                   = frame2jpg_cb(fb, 60, jpg_encode_stream, &jchunk) ? ESP_OK : ESP_FAIL;
...
#endif
            ,
            (uint32_t)(_jpg_buf_len), (uint32_t)frame_time, 1000.0f / (uint32_t)frame_time, avg_frame_time,
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
    ...
    if (!strcmp(variable, "framesize")) {
        if (s && (s->id.PID == GC0308_PID || s->id.PID == OV3660_PID) && val >= FRAMESIZE_96X96 &&
            val <= camera_max_frame_size()) {
            res = camera_reconfigure((framesize_t)val) ? 0 : -1;
        } else {
            log_e("Denied framesize change for sensor PID 0x%04x", s ? s->id.PID : 0);
            res = -1;
        }
...
static esp_err_t status_handler(httpd_req_t *req)
{
    ...
    *p++    = '{';
    ...
    p += sprintf(p, "\"sensor\":\"%s\",", sensor_name);
    p += sprintf(p, "\"max_framesize\":%u,", (unsigned)camera_max_frame_size());
...
static esp_err_t index_handler(httpd_req_t *req)
{
    ...
    httpd_resp_set_hdr(req, "Cache-Control", "no-store, no-cache, must-revalidate");
    sensor_t *s = esp_camera_sensor_get();
    if (s && (s->id.PID == GC0308_PID || s->id.PID == OV3660_PID)) {
...
static esp_err_t qr_handler(httpd_req_t *req)
{
    ...
    if (sensor && sensor->id.PID == OV3660_PID) {
        httpd_resp_send_err(req, HTTPD_501_METHOD_NOT_IMPLEMENTED, "QR scanning is unavailable for OV3660 JPEG frames");
    ...
    while (true) {
        ...
        qr_time /= 1000;
        log_i("MJPG: %uB %ums (%.1ffps) QR %ums", (uint32_t)(_jpg_buf_len), frame_time, 1000.0f / frame_time, qr_time,
              1000.0f / qr_time);
        camera_frame_unlock();
    }
    return res;
